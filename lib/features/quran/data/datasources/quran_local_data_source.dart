...
abstract class QuranLocalDatasource {
  ...
  Future<SurahModel> getSurah(Surah surah);
...
class QuranLocalDatasourceImpl implements QuranLocalDatasource {
  ...
  @override
  Future<SurahModel> getSurah(Surah surah) async {
    try {
      final number = int.parse(surah.index);
      final surahDetails = await rootBundle.loadString('assets/quran/surahs/surah_$number.json');
      final verses = List<VerseModel>.from(json.decode(surahDetails)['verse'].entries.map((e) => VerseModel.fromEntry(e)));
